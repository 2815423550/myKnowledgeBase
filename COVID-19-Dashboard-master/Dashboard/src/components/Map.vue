...
export default {
  ...
  computed: {
    ...
    locations() {
      const withConfirmedData = this.data.filter(i => i.confirmed > 0);
      console.log(withConfirmedData);
      const max = Math.max(
        ...withConfirmedData.map(item => {
      ...
      return withConfirmedData.map(item => ({
        ...item,
        radius: this.scale(item.confirmed, max)
      }));
    },
    tileLayerUrl() {
  ...
  methods: {
    ...
    scale(d, max) {
      const min = 5;
      const factor = 400;
      const zoomFactor = this.zoom >= 5 ? 1 : this.zoom / 8; // adjust divisor for best optics
      // console.log(Math.floor((d / 80000) * factor * zoomFactor) + min);
      return Math.floor((d * factor * zoomFactor) / max) + min;
    },
    getUserLocation() {
